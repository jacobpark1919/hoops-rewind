...
hoopsrewind.app`;
  ...
  return (
    <div className="fixed inset-0 bg-background flex items-center justify-center z-50 animate-slide-up overflow-hidden sm:overflow-hidden" onWheel={(e) => e.stopPropagation()} onTouchMove={(e) => e.stopPropagation()}>
      <div className="text-center max-w-md sm:max-w-3xl mx-auto px-3 sm:px-6 py-3 sm:py-4 w-full max-h-[100dvh] overflow-y-auto sm:overflow-y-hidden overscroll-contain">
        {won ? (
          <>
            <div className="w-10 h-10 sm:w-14 sm:h-14 mx-auto mb-1.5 sm:mb-2 rounded-full bg-success/20 flex items-center justify-center">
              <Trophy className="w-5 h-5 sm:w-7 sm:h-7 text-success" />
            </div>
          ...
          <>
            <h2 className="font-display text-xl sm:text-3xl font-bold text-foreground mb-1 sm:mb-1">
              Next time!
            </h2>
            <p className="font-display text-base sm:text-lg text-foreground mb-2 sm:mb-3">
        ...
        <div className="sm:grid sm:grid-cols-2 sm:gap-4 sm:items-start">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2 sm:gap-3 mb-2 sm:mb-3">
              <div className="bg-card rounded-lg sm:rounded-xl p-2 sm:p-3 border border-border">
                <p className="text-lg sm:text-2xl font-display font-bold text-accent">{correctCount}</p>
            ...
            {/* Date & Puzzle number */}
            <p className="text-foreground text-xs sm:text-sm font-medium mb-2">
              {today} &nbsp;·&nbsp; Puzzle #{puzzleNum}
            </p>

            {/* Emoji Result Grid */}
            <div className="bg-card rounded-lg sm:rounded-xl p-2 sm:p-3 border border-border mb-2 sm:mb-0">
              <p className="text-[10px] sm:text-xs text-muted-foreground uppercase tracking-wider mb-1 sm:mb-2">Your Results</p>
              <div className="flex justify-center gap-0.5 sm:gap-1 flex-wrap mb-1 sm:mb-2">
                <span className="text-base sm:text-xl animate-bounce-in" style={{ animationDelay: '0ms' }}>🟩</span>
                {resultHistory.map((correct, index) => (
                  <span 
                    key={index} 
                    className="text-base sm:text-xl animate-bounce-in"
          ...
          <div className="flex flex-col gap-1.5 sm:gap-2">
            ...
            <Button
              ...
              size="lg"
              variant="outline"
              className="w-full font-display text-xs sm:text-sm h-8 sm:h-9"
            >
              {copied ? (
                <>
            ...
            {typeof navigator !== 'undefined' && typeof navigator.share === 'function' && (
              <Button
                ...
                size="lg"
                variant="outline"
                className="w-full font-display text-xs sm:text-sm h-8 sm:h-9 border-accent text-accent hover:bg-accent hover:text-accent-foreground"
              >
            ...
            {user && onPlayPastPuzzle && (
              <Button
                ...
                size="lg"
                className="w-full font-display text-xs sm:text-sm h-8 sm:h-9 bg-accent text-accent-foreground hover:bg-accent/90"
              >
